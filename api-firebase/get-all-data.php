...
foreach ($result as $row) {
    ...
    if (!empty($product) || !empty($product_ids)) {
        ...
        $sql = 
         "SELECT p.*,s.name as seller_name,s.status as seller_status FROM `products`p JOIN seller s ON s.id = p.seller_id WHERE (p.is_approved = 1 AND  p.`status` = 1 AND s.status = 1 AND p.id IN ($product_ids))" . $and . " $sort LIMIT $offset,$limit";
        $db->sql($sql);
        $result1 = $db->getResult();
...
// seller
$res_seller = array();
$sql_query = "select s.* from products p join seller s on s.id=p.seller_id WHERE s.status = 1 and p.status=1  GROUP by p.seller_id ";
$db->sql($sql_query);
$result = $db->getResult();
$tempRow = array();
